...
func TestActorController_DeleteActor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			actorID: 1,
			setupMock: func(mas *MockActorService) {
				// Настраиваем ожидание вызова GetByID для проверки существования актёра
				mas.On("GetByID", 1).Return(domain.Actor{ID: 1}, nil)
				// Настраиваем ожидание вызова GetMovies с actorID=1, возвращаем пустой список фильмов
				mas.On("GetMovies", 1).Return([]domain.Movie{}, nil)
				// Настраиваем ожидание вызова Delete с actorID=1
		...
		},
		{
			...
			setupMock: func(mas *MockActorService) {
				// Настраиваем ожидание вызова GetByID для проверки существования актёра
				mas.On("GetByID", 1).Return(domain.Actor{ID: 1}, nil)
				// Настраиваем ожидание вызова GetMovies, возвращаем список с одним фильмом
		...
		},
		{
			...
			actorID: 999,
			setupMock: func(mas *MockActorService) {
				// Настраиваем ожидание вызова GetByID, возвращаем ошибку "actor not found"
...
func TestActorController_PartialUpdateActor(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := controller.PartialUpdateActor(&gin.Context{}, tt.actorID, tt.update)

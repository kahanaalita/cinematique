...
// Вспомогательные функции для создания указателей уже определены в actor_controller_test.go
...
func TestMovieController_SearchMoviesByTitle(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			query: "",
			setupMock: func(mms *MockMovieService) {
				mms.On("SearchMoviesByTitle", "").Return([]domain.Movie{}, nil)
			},
			expectedResult: dto.MoviesListResponse{
				Movies: []dto.MovieResponse{},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx.Request = &http.Request{
				URL: &url.URL{RawQuery: "query=" + tt.query},
			}
